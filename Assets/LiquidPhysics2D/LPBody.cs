...
{	
	...
	{
		//transform no zahyo 1124
		//transform.position = new Vector3(x,y,transform.position.z);
		//diff = Mathf.Rad2Deg*ang;
		//transform.localEulerAngles = new Vector3(0f,0f,diff+StartRotation);
		//close calculation of the plugins
		LPAPIBody.SetBodyTransform (this.ThingPtr,transform.position.x, transform.position.y, 0.0f); //1124
	}
	...
	//1116
	public void SetBody(GameObject tem)
	{
		
		//GameObject tem = GameObject.Find("Hand");

		LPAPIBody.SetBodyTransform (this.ThingPtr,tem.transform.position.x, tem.transform.position.y, 0.0f);
	}
